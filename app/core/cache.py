...
def news_key(category: str, limit: int, israeli_only: bool, exclude_negative: bool, language: str = "english", user_tier: str = "free", with_analysis: bool = False) -> str:
    return f"news:{category}:{limit}:{int(israeli_only)}:{int(exclude_negative)}:{language.lower()}:{user_tier.lower()}:{int(with_analysis)}"
